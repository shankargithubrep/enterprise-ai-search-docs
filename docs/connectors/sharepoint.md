...
| `tenant_name` | string | required | Your SharePoint tenant name (e.g. `acme` for `acme.sharepoint.com`). |
...
| `site_collections` | list of strings | `[]` (all sites) | Allowlist of site collection URLs. Empty = crawl all sites the app can access. For multi-tenant the customer: set to specific tenant site. |
...
Create one app registration per the deployment region (not per tenant — one app covers all SharePoint tenants accessible in that Azure AD).
...
> **Principle of least privilege note:** `Files.Read.All` grants access to ALL files across ALL sites in the tenant. For multi-tenant deployments where each "tenant" is a separate SharePoint tenant (separate Azure AD), this is fine. If multiple enterprise customers share one Azure AD tenant, use `site_collections` allowlist to scope access.
...
For multi-tenant deployments, each customer tenant has its own SharePoint Online subscription (separate Azure AD). This means:
